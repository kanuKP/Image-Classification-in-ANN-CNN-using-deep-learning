...
img = gdal.Open("F:/CNN/17/test/apex17bnads")
b = img.RasterCount
col = img.RasterXSize
row = img.RasterYSize
...
def train():
    ...
    print(os.listdir("F:/CNN/17/train1"))
    path = os.listdir("F:/CNN/17/train1")
    pixels = row * col
    y_test = np.zeros([row * col], dtype=np.uint16)
    x_test = ReadBilFile("F:/CNN/17/test/apex17bnads", bands, pixels)
